...
func runCmake(directory string, params string) (string, error) {
	...
	cmakelistsPath := filepath.Join(config.SDKDirectory, "scripts", "CMake", "Sparks", "CMakeLists.txt")
	params += fmt.Sprintf("\"%s\"", cmakelistsPath)
	var output string
	if output, err := utils.ExecuteEx("cmake", directory, true, cmakelistsPath); err != nil {
